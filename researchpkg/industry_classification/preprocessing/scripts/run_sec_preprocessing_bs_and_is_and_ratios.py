GLOBAL_EXP_NAME = "_count30_sic1agg_including_is_with12ratios"
"""
...
def generate_bs_dataset(
    ...
    for (adsh, group,) in tqdm.tqdm(
        ...
        bs_calculated_values = {k: v for k, v in bs_calculated_values.items() if v != 0}
        ...
        is_calculated_values = {k: v for k, v in is_calculated_values.items() if v != 0}
        ...
        new_tags = [k for k, v in all_calculated_values.items() if k not in values_dict]
        ...
        ratios12_values = computeRatios12(values_dict)
        ratios12_values = {k: v for k, v in ratios12_values.items() if v != 0}

        # Normalize the values dict by assets
        assets = values_dict["Assets"]
        values_dict = {k: v / assets for k, v in values_dict.items()}

        values_dict.update(ratios12_values)

        nline = len(values_dict)
        nline_raw_tag = len(group.tag.unique())
    ...
    df_merged = pd.merge(
        ...
        on="tag",
        how="left",
    ).drop_duplicates(subset=["adsh", "tag"])

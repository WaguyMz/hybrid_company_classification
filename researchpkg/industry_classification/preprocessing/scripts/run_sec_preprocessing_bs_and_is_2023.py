...
def generate_bs_dataset(
    ...
    df_merged["net_change"] = df_merged["net_change"] * df_merged["value_sign"]
    df_merged.drop(
        columns=["value"],
        inplace=True,
    )
    ...
    is_calculation_tree = CalculationTree.build_taxonomy_tree(
        SEC_TAX_DATA_DIR,
        SEC_TAX,
        SEC_TAX_VERSION,
    ...
    def get_tag_depth(
        ...
        node = node or is_calculation_tree.get_node_by_concept_name(x)
        if node != None:
            return node.level
        else:
            return -1
    ...
    df_merged["nline"] = df_merged.groupby("adsh")["nline"].transform("first")

    df_merged = df_merged.loc[df_merged.nline >= TAGS_PERSHEET_COUNT_THRESHOLD]

    print("Nb is tags : ", df_merged.query("stmt == 'IS'").tag.nunique())
    ...
    if fill_missing_values:
        for (adsh, group,) in tqdm.tqdm(
            ...
            bs_calculated_values = bs_calculation_tree.apply(
                values_dict,
                min_level=SEC_TAX_MIN_TAGS_DEPTH,
                max_level=SEC_TAX_MAX_TAGS_DEPTH,
            )
            bs_calculated_values = {
                k: v for k, v in bs_calculated_values.items() if v != 0
            }

            is_calculated_values = is_calculation_tree.apply(
                {**values_dict, **bs_calculated_values},
                min_level=SEC_TAX_MIN_TAGS_DEPTH,
                max_level=SEC_TAX_MAX_TAGS_DEPTH,
            )
            is_calculated_values = {
                k: v for k, v in is_calculated_values.items() if v != 0
            }

            all_calculated_values = {**bs_calculated_values, **is_calculated_values}

            new_bs_tags = [
                k for k, v in bs_calculated_values.items() if k not in values_dict
            ]
            new_is_tags = [
                k for k, v in is_calculated_values.items() if k not in values_dict
            ]

            new_tags = [
                k for k, v in all_calculated_values.items() if k not in values_dict
            ]

            if KEEP_ONLY_TOP_TAGS:
                new_tags = [k for k in new_tags if k in top_k_tags.index]
                values_dict.update(
                    {k: v for k, v in bs_calculated_values.items() if k in new_tags}
                )
            else:
                values_dict = all_calculated_values
    ...
    avg_nb_new_bs_tags = np.mean(nb_new_bs_tags).item()
    print(
        "Average number of new bs tags per sheet",
        avg_nb_new_bs_tags,
    )

    avg_nb_new_is_tags = np.mean(nb_new_is_tags).item()
    print(
    ...
    df_merged["nb_sub"] = df_merged.groupby("cik")["adsh"].transform("nunique")
    ...
    df_merged["sub"] = 1
    df_merged["cum_nb_sub"] = df_merged["sub"].cumsum()
    df_merged["cum_nb_sub"] = df_merged.groupby("cik")["cum_nb_sub"].transform("max")

    total_nb_submissions = df_merged.adsh.nunique()

    nb_validation_samples = total_nb_submissions * SEC_VALIDATION_RATIO
    nb_test_samples = total_nb_submissions * SEC_TEST_RATIO

    df_merged["test_flag"] = df_merged.cum_nb_sub <= nb_test_samples
    df_merged["train_flag"] = (
        df_merged.cum_nb_sub >= nb_test_samples + nb_validation_samples
    )
    ...
    save_dataset_config(
        ...
        nb_cik_train=df_data[df_data.train_flag].cik.nunique(),
        nb_cik_val=df_data[
            ((~df_data.train_flag) & (~df_data.test_flag))
        ].cik.nunique(),
        nb_cik_test=df_data[df_data.test_flag].cik.nunique(),
        ...
        avg_nb_new_bs_tags_per_sheet=avg_nb_new_bs_tags,
        avg_nb_new_is_tags_per_sheet=avg_nb_new_is_tags,
    ...
    return df_merged, tags_index, accounts_index, submissions_index
...
if __name__ == "__main__":
    ...
    df_merged = merge_all_directories_data(
        SEC_RAW_DATA_DIR,
        min_year=2023,
        max_year=2024,
        only_balance_sheet_tags=False,
    )
    print("Size of the merged dataframe: ", df_merged.shape)
    print("Number of submission: ", df_merged.adsh.nunique())
    print("Number of cik", df_merged.cik.nunique())
